...
class Novel {
    ...
    static async mkdirSingleNovel(targetpath, id, name) {
        try {
            await fs.stateAsync(path.join(targetpath, `./novels/${id}-${name}`));
            // await fs.statAsync(`./novels/${id}-${name}`)
        } catch (e) {
            await fs.mkdirAsync(path.join(targetpath, `./novels/${id}-${name}`), { recursive: true });
     ...
     * @memberof Novel
     */
    static async download(targetpath, url) {
        if (!fs.existsSync(path.join(targetpath, "novels"))) {
            fs.mkdirSync(path.join(targetpath, "novels"));
        }
        ...
        Novel.mkdirSingleNovel(targetpath, novel.id, novel.name); // 创建本地目录
...
module.exports = (options, callback) => {
    ...
    Novel.download(path, options.url)
    callback()
};
